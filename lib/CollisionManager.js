...
export class CollisionManager {
    ...
    doCollideCircleToCircle(collider1, collider2) {
        ...
        collider1.velocity.x -= (collider1.velocity.x - collider1.prevVelocity.x)*inverseMass1;
        collider1.velocity.y -= (collider1.velocity.y - collider1.prevVelocity.y)*inverseMass1;
        collider2.velocity.x -= (collider2.velocity.x - collider2.prevVelocity.x)*inverseMass2;
        collider2.velocity.y -= (collider2.velocity.y - collider2.prevVelocity.y)*inverseMass2;
    ...
    doCollideCircleToBox(collider1, collider2) {
        ...
        boxCollider.velocity.x -= (boxCollider.velocity.x - boxCollider.prevVelocity.x)*inverseMassBox;
        boxCollider.velocity.y -= (boxCollider.velocity.y - boxCollider.prevVelocity.y)*inverseMassBox;
        circleCollider.velocity.x -= (circleCollider.velocity.x - circleCollider.prevVelocity.x)*inverseMassCircle;
        circleCollider.velocity.y -= (circleCollider.velocity.y - circleCollider.prevVelocity.y)*inverseMassCircle;
        ...
        $.shape.oval(boxPoint.x, boxPoint.y, 3);
        ...
        console.log(boxCollider.rotationalVelocity);

        if (boxCollider.static == false) {

            let rotVel = {x: boxPointVelocity.x + circleCollider.velocity.x, y:boxPointVelocity.y + circleCollider.velocity.y};
            let rotNormal = CollisionUtilities.getNormal(boxCollider.x - boxPoint.x, boxCollider.y - boxPoint.y);
            let dot = CollisionUtilities.dotProduct(rotVel, rotNormal);
            let direction = 0;
            if (dot > 0) {
                direction = -1;
            } else if (dot < 0) {
            ...
            boxCollider.rotationalVelocity = direction * CollisionUtilities.distance(rotVel.x, rotVel.y) * boxPercentage;
            console.log(boxCollider.rotationalVelocity);
            ...
            let bounce = boxCollider.bounciness/100;

            boxCollider.velocity.x = boxCollider.velocity.x * (Math.max(1 - Math.abs(normalVector.x), bounce));
            boxCollider.velocity.y = boxCollider.velocity.y * (Math.max(1 - Math.abs(normalVector.y), bounce));
        ...
        if (circleCollider.static == false) {
            ...
            vectorV.x += boxPointVelocity.x;
            vectorV.y += boxPointVelocity.y;
        ...
        let normilizedVector = CollisionUtilities.normalize(circleCollider.x - boxPoint.x, circleCollider.y - boxPoint.y);
        let collides = true;
        while(collides) {

            if (boxCollider.static == false) {
                boxCollider.x += boxCollider.velocity.x * inverseMassBox * (1-boxPercentage);
                boxCollider.y += boxCollider.velocity.y * inverseMassBox * (1-boxPercentage);
                boxCollider.rotation += boxCollider.rotationalVelocity * inverseMassBox * boxPercentage;
                boxCollider.updateBounds();
            }
            if (circleCollider.static == false) {
                circleCollider.x += circleCollider.velocity.x * inverseMassCircle;
                circleCollider.y += circleCollider.velocity.y * inverseMassCircle;
            }
            collides = CollisionUtilities.rectangleIntersectsCircle(circleCollider.x, circleCollider.y, circleCollider.radius, 
                boxCollider.vertices[0].x, boxCollider.vertices[0].y, 
                boxCollider.vertices[1].x, boxCollider.vertices[1].y,
                boxCollider.vertices[2].x, boxCollider.vertices[2].y, 
                boxCollider.vertices[3].x, boxCollider.vertices[3].y);
