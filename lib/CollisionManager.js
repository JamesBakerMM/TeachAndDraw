...
export class CollisionManager {
    ...
    doCollideCircleToBox(collider1, collider2) {
        ...
        if (boxCollider.static == false) {
            ...
            boxCollider.velocity.x = boxCollider.velocity.x * (Math.max(1 - Math.abs(normalVector.x) - bounce, 0));
            boxCollider.velocity.y = boxCollider.velocity.y * (Math.max(1 - Math.abs(normalVector.y) - bounce, 0));
        ...
        if (circleCollider.static == false) {
            ...
            circleCollider.velocity.x = circleCollider.velocity.x * (Math.max(1 - Math.abs(normalVector.x) - bounce, 0));
            circleCollider.velocity.y = circleCollider.velocity.y * (Math.max(1 - Math.abs(normalVector.y) - bounce, 0));
        }
