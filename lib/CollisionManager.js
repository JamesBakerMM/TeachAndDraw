...
export class CollisionManager {
    ...
    doCollideCircleToBox(collider1, collider2) {
        ...
        const boxRadius = CollisionUtilities.distanceBetweenpoints(boxPoint, intersectLinePoint);
        let difference = (circleCollider.radius + boxRadius + CollisionUtilities.ERROR - CollisionUtilities.distanceBetweenpoints(boxPoint, circleCollider)) / 2;
        if (point != null) {
            const cornerToCircle = ((circleCollider.radius + CollisionUtilities.ERROR) - CollisionUtilities.distanceBetweenpoints(point, circleCollider)) / 2;
            difference = Math.max(difference, cornerToCircle);
        }

        if (boxCollider.static == false) {
            boxCollider.x += normilizedVector.x * difference * inverseMassBox;
            boxCollider.y += normilizedVector.y * difference * inverseMassBox;
        }
        if (circleCollider.static == false) {
            circleCollider.x -= normilizedVector.x * difference * inverseMassCircle;
            circleCollider.y -= normilizedVector.y * difference * inverseMassCircle;
        }
    }
    ...
    doCollideBoxToBox(collider1, collider2) {
        ...
        $.shape.oval(intersectLinePoint.x, intersectLinePoint.y, 3);
        $.shape.line(intersectLine.x1, intersectLine.y1, intersectLine.x2, intersectLine.y2);
