...
export class CollisionManager {
    ...
    doCollideCircleToCircle(circle1, circle2) {
        ...
        let angle1 = 0;
        if (circle1.velocity.x != 0 || circle1.velocity.y != 0)
        {
        ...
        let angle2 = 0;
        if (circle2.velocity.x != 0 || circle2.velocity.y != 0)
        {
            angle2 = CollisionUtilities.angleBetweenVectors(new Vector(circle2.velocity.x, circle2.velocity.y), new Vector(vectorBetween.x, vectorBetween.y));
        }
    ...
    doCollideCircleToPolygon(circle, polygon) {
        ...
        let rotationToAddToPolygon = rotationalForce.length() * percentage * (0.05 / circle.torque);
        ...
        if (!circle.static) {
            const mult = inverseMass1 * pushVector;
            const bounce = circle.bounciness / 105;
            const grip = circle.grip / 105;

            circle.collisionShape.newVelocityX += (vectorBetween.x - (pushForce.x * grip)) * mult * bounce;
            circle.collisionShape.newVelocityY += (vectorBetween.y - (pushForce.y * grip)) * mult * bounce;
            circle.collisionShape.newVelocityRot += rotationToAddToCircle * grip * mult;
        }
        
        if (!polygon.static) {
            const mult = inverseMass2 * pushVector;
            const bounce = polygon.bounciness / 105;
            const grip = polygon.grip / 105;

            polygon.collisionShape.newVelocityX -= (vectorBetween.x + (pushForce.x * grip)) * mult * bounce;
            polygon.collisionShape.newVelocityY -= (vectorBetween.y + (pushForce.y * grip)) * mult * bounce;
            polygon.collisionShape.newVelocityRot += rotationToAddToPolygon * grip * mult;
        }
    ...
    doCollideBoxToBox(collider1, collider2) {
        ...
        if (!collider1.static) {
            const mult = inverseMass1 * pushVector;
            const bounce = collider1.bounciness / 105;
            const grip = collider1.grip / 105;
            collider1.collisionShape.newVelocityX += (vectorBetween.x - (pushForce.x * grip)) * mult * bounce;
            collider1.collisionShape.newVelocityY += (vectorBetween.y - (pushForce.y * grip)) * mult * bounce;
            collider1.collisionShape.newVelocityRot += rotationToAddToPolygon1 * grip * mult;
        }
        
        if (!collider2.static) {
            const mult = inverseMass2 * pushVector;
            const bounce = collider2.bounciness / 105;
            const grip = collider2.grip / 105;
            collider2.collisionShape.newVelocityX -= (vectorBetween.x + (pushForce.x * grip)) * mult * bounce;
            collider2.collisionShape.newVelocityY -= (vectorBetween.y + (pushForce.y * grip)) * mult * bounce;
            collider2.collisionShape.newVelocityRot += rotationToAddToPolygon2 * grip * mult;
        }
