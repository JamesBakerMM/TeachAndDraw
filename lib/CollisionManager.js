...
export class CollisionManager {
    ...
    doCollide(collider1, collider2) {
        ...
        if (collider1.static == false && collider2.static == false) {
            ...
            let angle1 = 0;
            if (CollisionUtilities.distance(v1.x, v1.y) > 0) {
                angle1 = CollisionUtilities.angleBetweenVectors(CollisionUtilities.normalize(v1.x, v2.y), CollisionUtilities.normalize(-vectorBetween.x, -vectorBetween.y));
            ...
            let angle2 = 0;
            if (CollisionUtilities.distance(v2.x, v2.y) > 0) {
                angle2 = CollisionUtilities.angleBetweenVectors(CollisionUtilities.normalize(v2.x, v2.y), CollisionUtilities.normalize(vectorV.x, vectorV.y));
            ...
            collider1.velocity.modify(impulseVec.x * m1 * extraSpinFraction2, impulseVec.y * m1 * extraSpinFraction2);
            collider2.velocity.modify(-impulseVec.x * m2 * extraSpinFraction1, -impulseVec.y * m2 * extraSpinFraction1);
            ...
            collider1.rotationalVelocity += extraSpinFraction2 / 10;

            collider2.rotationalVelocity = rot2;
            collider2.rotationalVelocity += extraSpinFraction1 / 10;
