...
export class CollisionManager {
    ...
    collides(collider1, collider2) {
        ...
        //AABB check
        if (collider1.x2 < collider2.x1) {
            return false;
        }
        if (collider2.x2 < collider1.x1) {
            return false;
        }
        if (collider1.y2 < collider2.y1) {
            return false;
        }
        if (collider2.y2 < collider1.y1) {
            return false;
        }
    ...
    doCollide(collider1, collider2) {
        ...
        collider1.velocity.modify(impulseVec.x * m1, impulseVec.y * m1,this.#pen.time);
        collider2.velocity.modify(-impulseVec.x * m2, -impulseVec.y * m2,this.#pen.time);
