...
export class CollisionManager {
    ...
    constructor(pen){
        ...
        this.#collidesList = new Array(5000);
    }
    ...
    collides(collider1, collider2) {
        /*
        if (collider1.quadValue < 0) {
            collider1.quadValue = this.#pen.quadTree.getValue(collider1.x, collider1.y, collider1.w, collider1.h);
        }

        if (collider2.quadValue < 0) {
            collider2.quadValue = this.#pen.quadTree.getValue(collider2.x, collider2.y, collider2.w, collider2.h);
        }

        if ((collider1.quadValue & collider2.quadValue) == 0) {
            return false;
        }
        */
    ...
    handleCollisions() {   
        for (let i = 0; i < this.#collisionCounter; i += 2) {
            const entry1 = this.#collidesList[i];
            const entry2 = this.#collidesList[i+1];
            entry1.quadValue = -1;
            entry2.quadValue = -1;
