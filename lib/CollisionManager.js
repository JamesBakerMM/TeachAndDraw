    ...
    doCollide(collider1, collider2) {
        ...
        if (collider1.shape == "circle" && collider2.shape == "circle") {
            ...
            if (boxCollider.w == boxCollider.h) {
                ...
                let length = 0;
                if (boxCollider.w > boxCollider.h) {
                    m1 = (boxCollider.vertices[1].y - boxCollider.vertices[0].y) / (boxCollider.vertices[1].x - boxCollider.vertices[0].x); //dy/dx
                    length = boxCollider.w;
                } else if (boxCollider.h > boxCollider.w) {
                    m1 = (boxCollider.vertices[2].y - boxCollider.vertices[0].y) / (boxCollider.vertices[2].x - boxCollider.vertices[0].x); //dy/dx
                    length = boxCollider.h;          
                }
                //work out the vector for that axis
                let vectorAxis = {x: 0,  y: 1};
                if (Number.isFinite(m1)) {
                    const c1 = boxCollider.y - (m1 * boxCollider.x); //c = y - mx
                    const x1 = (boxCollider.x + 100);
                    const y1 = (m1 * x1) + c1;
                    vectorAxis = CollisionUtilities.normalize(x1-boxCollider.x, y1-boxCollider.y);
                }
                const vectorAxisNormal = {x: vectorAxis.y,  y: -vectorAxis.x};

                let intersect = CollisionUtilities.lineIntersectsLine(boxCollider.x, boxCollider.y, boxCollider.x + vectorAxis.x, boxCollider.y + vectorAxis.y,
                circleCollider.x, circleCollider.y, circleCollider.x + vectorAxisNormal.x, circleCollider.y + vectorAxisNormal.y);
                ...
                if (dist > (length/2)) {
                    ...
                    let percentage = ((length/2) / dist) * dist;
                ...
                $.colour.stroke = "black";
                $.shape.line(boxCollider.x - (vectorAxis.x * 100), boxCollider.y - (vectorAxis.y * 100), boxCollider.x + (vectorAxis.x * 100), boxCollider.y + (vectorAxis.y * 100));
                $.shape.line(boxCollider.x + (vectorAxisNormal.x * 30), boxCollider.y + (vectorAxisNormal.y * 30), boxCollider.x - (vectorAxisNormal.x * 30), boxCollider.y - (vectorAxisNormal.y * 30));
            }
            
        } else if (collider1.shape == "box" && collider2.shape == "box") {
            //intersection is the average point of all line intersects.

            //if there are no line intersects (one inside the other), just use the circle intersect instead
            axis1 = { x: collider1.x, y: collider1.y};
            axis2 = { x: collider2.x, y: collider2.y};
        }

        $.shape.oval(axis1.x, axis1.y, 10);
        $.shape.oval(axis2.x, axis2.y, 10);

        let vectorN = CollisionUtilities.normalize(axis1.x - axis2.x, axis1.y - axis2.y);

        let m1 = (2 * collider2.mass) / (collider1.mass + collider2.mass);
        let m2 = (2 * collider1.mass) / (collider2.mass + collider1.mass);    
        ...
        if (collider1.static == false) {
            ...
            let mult = m1 * ((CollisionUtilities.dotProduct(vectorV, vectorBetween)) / CollisionUtilities.dotProduct(vectorBetween, vectorBetween));
        ...
        if (collider2.static == false) {
            ...
            let mult = m2 * ((CollisionUtilities.dotProduct(vectorV, vectorBetween)) / CollisionUtilities.dotProduct(vectorBetween, vectorBetween));
        ...
        //move objects apart to prevent collision over multiple frames
        let dist = CollisionUtilities.distanceBetweenpoints(axis1, axis2);
        let minimumDist = 0;
        const buffer = 0.5;
        if (collider1.shape == "circle" && collider2.shape == "circle") {
            minimumDist = collider1.radius + collider2.radius + buffer;
        } else if (collider1.shape == "box" && collider2.shape == "circle") {
            minimumDist = (collider1.h/2) + collider2.radius + buffer;
        } else if (collider1.shape == "circle" && collider2.shape == "box") {
            minimumDist = collider1.radius + (collider2.h/2) + buffer;
        } else if (collider1.shape == "box" && collider2.shape == "box") {
            minimumDist = collider1.radius + collider2.radius + buffer;
        }
