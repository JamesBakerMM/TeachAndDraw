...
export class CollisionManager {
    ...
    collidersOverlap(collider1, collider2) {
        ...
        if (collider1.shape === "circle" && collider2.shape === "circle") {
            ...
            return collider1.collisionShape.doesCircleIntersect(collider2);

        } else if (collider1.shape === "circle" && collider2.shape === "box") {
            return collider2.collisionShape.doesCircleIntersect(collider1);
    ...
    doCollideCircleToCircle(circle1, circle2) {
        ...
        if (!circle1.static) {
            ...
            circle1.velocity.x += vectorBetween.x * mult;
            circle1.velocity.y += vectorBetween.y * mult;
        }
        
        if (!circle2.static) {
            const mult = inverseMass2 * pushVector;
            circle2.velocity.x -= vectorBetween.x * mult;
            circle2.velocity.y -= vectorBetween.y * mult;
        }
    ...
    doCollideCircleToPolygon(circle, polygon) {
        ...
        if (!circle.static) {
            const mult = inverseMass1 * pushVector;
            circle.velocity.x += vectorBetween.x * mult;
            circle.velocity.y += vectorBetween.y * mult;
        }
        
        if (!polygon.static) {
            const mult = inverseMass2 * pushVector;
            polygon.velocity.x -= vectorBetween.x * mult;
            polygon.velocity.y -= vectorBetween.y * mult;
        }
        ...
        while(collides && count < 1000) {
            ...
            if (circle.static == false) {
                circle.x -= normilizedVector.x * inverseMass1;
                circle.y -= normilizedVector.y * inverseMass1;
                circle.updateCollision();
            }
            if (polygon.static == false) {
                polygon.x += normilizedVector.x * inverseMass2;
                polygon.y += normilizedVector.y * inverseMass2;
                polygon.updateCollision();
            }
            collides = polygon.collisionShape.doesCircleIntersect(circle);
        }
    }
    ...
    doCollideBoxToBox(collider1, collider2) {
        ...
        while(collides && count < 100) {
            ...
            collides = collider2.collisionShape.doesPolygonIntersect(collider1.collisionShape);
        }
