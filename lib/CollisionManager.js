...
export class CollisionManager {
    ...
    static getAxisForBox(boxCollider, edge) {
        let axis = {hasAxis: false, length: 0, height: 0, m: 0, c: 0, vector:{x:0, y:0}, normal:{x:0, y:0}}
    ...
    doCollide(collider1, collider2) {
        ...
        if (collider1.shape == "circle" && collider2.shape == "circle") {
            ...
            if (boxCollider.w == boxCollider.h) {
                ...
                let edge = CollisionUtilities.getEdgeLineGoesThrough(circleCollider.x, circleCollider.y, boxCollider.x, boxCollider.y,  
                ...
                const boxAxis = CollisionManager.getAxisForBox(boxCollider, edge);
            ...
            const edge1 = CollisionUtilities.getEdgeLineGoesThrough(collider1.x, collider1.y, intersect.x, intersect.y,  
                collider1.vertices[0].x, collider1.vertices[0].y, 
                collider1.vertices[1].x, collider1.vertices[1].y,
                collider1.vertices[2].x, collider1.vertices[2].y, 
                collider1.vertices[3].x, collider1.vertices[3].y);

            const box1Axis = CollisionManager.getAxisForBox(collider1, edge1);
            ...
            const edge2 = CollisionUtilities.getEdgeLineGoesThrough(collider2.x, collider2.y, intersect.x, intersect.y,  
                collider2.vertices[0].x, collider2.vertices[0].y, 
                collider2.vertices[1].x, collider2.vertices[1].y,
                collider2.vertices[2].x, collider2.vertices[2].y, 
                collider2.vertices[3].x, collider2.vertices[3].y);

            const box2Axis = CollisionManager.getAxisForBox(collider2, edge2);
...
*/
        ...
        if (collider1.shape == "circle" && collider2.shape == "circle") {
            ...
            vectorN = CollisionUtilities.normalize(collider2.x - intersect.x, collider2.y - intersect.y);
            difference = collider2.radius - CollisionUtilities.distance(intersect.x - collider2.x, intersect.y - collider2.y) + buffer;
        } else if (collider1.shape == "circle" && collider2.shape == "box") {
            vectorN = CollisionUtilities.normalize(collider1.x - intersect.x, collider1.y - intersect.y);
            difference = collider1.radius - CollisionUtilities.distance(intersect.x - collider1.x, intersect.y - collider1.y) + buffer;
        } else if (collider1.shape == "box" && collider2.shape == "box") {
            difference = collider1.radius + collider2.radius + buffer;
        }
