...
export class CollisionManager {
    ...
    addCollisionEvent(collider1, collider2) {
        ...
        if(!this.#collidesMap[newKey]){
            const collisionEvent=[collider1,collider2];
            this.#collidesMap[newKey]=collisionEvent;
            this.#collidesList.push(collisionEvent)
        }

        //prevway
        // for(let i = 0; i < this.#collidesList.length; i++) {
        //     let entry = this.#collidesList[i];
        //     if ((entry[0].id === collider1.id && entry[1].id === collider2.id) ||
        //         (entry[1].id === collider1.id && entry[0].id === collider2.id)) {
        //         return;
        //     }
        // }
    ...
    doCollide(collider1, collider2) {
        ...
        // let new_sepVel = -vectorSeparate * Math.min(collider1.bounciness, collider2.bounciness);
        ...
        collider1.velocity.x += (impulseVec.x * m1);
        collider1.velocity.y += (impulseVec.y * m1);
        collider2.velocity.x -= (impulseVec.x * m2);
        collider2.velocity.y -= (impulseVec.y * m2);
