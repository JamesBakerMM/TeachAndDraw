...
export class CollisionManager {
    ...
    doCollideCircleToCircle(circle1, circle2) {
        ...
        if (!circle1.static) {
            ...
            const bounce = ((circle1.bounciness / 2) + 50) / this.#SCALER_LIMIT;
            const grip = circle1.grip / this.#SCALER_LIMIT;
            const collisionCount = circle1.collisionShape.collisionCount;
        ...
        if (!circle2.static) {
            ...
            const bounce = ((circle2.bounciness / 2) + 50) / this.#SCALER_LIMIT;
            const grip = circle2.grip / this.#SCALER_LIMIT;
            const collisionCount = circle2.collisionShape.collisionCount;
    ...
    doCollideCircleToPolygon(circle, polygon) {
        ...
        if (!circle.static) {
            const mult = inverseMass1 * pushVector;
            const bounce = ((circle.bounciness / 2) + 50) / this.#SCALER_LIMIT;
            const grip = circle.grip / this.#SCALER_LIMIT;
            const collisionCount = circle.collisionShape.collisionCount;
        ...
        if (!polygon.static) {
            const mult = inverseMass2 * pushVector;
            const bounce = ((polygon.bounciness / 2) + 50) / this.#SCALER_LIMIT;
            const grip = polygon.grip / this.#SCALER_LIMIT;
            const collisionCount = polygon.collisionShape.collisionCount;
    ...
    doCollideBoxToBox(collider1, collider2) {
        ...
        if (!collider1.static) {
            const mult = inverseMass1 * pushVector;
            const bounce = ((collider1.bounciness / 2) + 50) / this.#SCALER_LIMIT;
            const grip = collider1.grip / this.#SCALER_LIMIT;
            const collisionCount = collider1.collisionShape.collisionCount;
        ...
        if (!collider2.static) {
            const mult = inverseMass2 * pushVector;
            const bounce = ((collider2.bounciness / 2) + 50) / this.#SCALER_LIMIT;
            const grip = collider2.grip / this.#SCALER_LIMIT;
            const collisionCount = collider2.collisionShape.collisionCount;
            ...
            if (isStatic == doStatics) {
                ...
                if (overlaps) {
                    ...
                    if (type === 1) { //1 MEANS COLLIDE
                        ...
                        c1.collisionShape.collisionCount += 1;
                        c2.collisionShape.collisionCount += 1;
