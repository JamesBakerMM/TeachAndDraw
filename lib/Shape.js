...
export class Shape {
    ...
    /**
     * Convert an Array<number> to an Array<Vector>. Should be removed later.
    ...
    #xyToVectors( points ) {
        ...
        for (let i=0; i<points.length; i+=2) {
            out.push(new Vector(points[i+0], points[i+1]));
        }
        return out;
    }
     ...
     * @param {Vector} B 
     * @param {Vector} C 
     * @returns {Vector[]} D, E, F points of triangle.
     */
    #computeRoundingDEF(A, B, C) {
        ...
        const BAhat = A.subtractVec(B);
              BAhat.normalize();

        const BChat = C.subtractVec(B);
              BChat.normalize();

        // Angle between BA and BC
        let theta = Math.acos(BChat.dot(BAhat));
        const sinTheta = Math.sin(0.5 * theta);
        const cosTheta = Math.cos(0.5 * theta);

        const Opposite = this.#rounding;
        const Hypotenuse = Opposite / sinTheta;
        const Adjacent = Hypotenuse * cosTheta;

        // Vector from B to D
        const BD = BAhat.clone();
              BD.multiply(Adjacent);

        // Vector from B to E
        const BE = BChat.clone();
              BE.multiply(Adjacent);

        // Vector pointing from B to F
        const BFhat = BAhat.addVec(BChat);
              BFhat.multiply(0.5);
              BFhat.normalize();

        // Vector from B to F
        const BF = BFhat.clone();
    ...
    #roundedMultiLine(coords) {
        const points = this.#xyToVectors(coords);
        ...
        ctx.save();
        this.#applyRotation(first.x, first.y);
        ctx.beginPath();
        ...
        for (let i=1; i<points.length-1; i++)
        {
            ...
            [D, E, F] = this.#computeRoundingDEF(A, B, C);

            ctx.lineTo(D.x, D.y);
            ctx.arcTo(B.x, B.y, E.x, E.y, this.rounding);
        }
    ...
    #roundedPolygon(coords) {
        ...
        for (let i=1; i<points.length-1; i++)
        {
            ...
            let [Dx, Dy, Ex, Ey, Fx, Fy] = [0, 0, 0, 0, 0, 0, 0];

            ctx.lineTo(D.x, D.y);
            ctx.arcTo(B.x, B.y, E.x, E.y, this.rounding);
        }

        [D, E, F] = this.#computeRoundingDEF(secondLast, last, first);
        ctx.lineTo(D.x, D.y);
        ctx.arcTo(last.x, last.y, E.x, E.y, this.rounding);
    
        [D, E, F] = this.#computeRoundingDEF(last, first, second);
        ctx.lineTo(D.x, D.y);
        ctx.arcTo(first.x, first.y, E.x, E.y, this.rounding);
    ...
    //  * @param {Array<number>} points 
    //  */
    // roundedMultiline(points) {
    //     this.#roundedMultiLine(this.#xyToVectors(points));
    //     return;
    // }
    ...
    //     this.#roundedPolygon(this.#xyToVectors(points));
    //     return;
    // }
