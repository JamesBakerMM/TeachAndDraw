...
export class Slider extends ShapedAssetEntity {
	...
	set step(value) {
		...
		// Throw error if value negative, not an integer, is NaN or Infinity
		if (Number.isFinite(value) === false) {
			throw new Error(
			...
			throw new Error(
				`The step has to be a whole number, meaning no decimal places. You gave ${value}.`
			)
		}
	...
	#drawPopup() {
		switch (this.#pen.gui.theme) {
			case "retro":
                ...
                if (this.#isAnimating && this.#animProgress < (animationSpeed * 10) / gradient) {
                    this.#animProgress += animationSpeed;
					animY = -((gradient * this.#animProgress - 5) ** 2) + 25;
                } else if (this.#animProgress === 10) {
                    this.#isAnimating = false;
