...
export class Slider extends ShapedAssetEntity {
	...
	set min(value) {
		...
		if (value < -1000000) {
			throw new Error(
				`Max must be above -1,000,000 (negative 1 million). ` +
				`You gave ${value} for min.`
			)
		}
		this.#min = value;
	}
	...
	#drawPopup() {
		switch (this.#pen.gui.theme) {
			case "retro":
				...
				const popupWidth = numChars * 10 + 3; // Add approx 10 pixels per char for this font
