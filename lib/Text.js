...
export class Text {
     ...
     */
    _addHyphenatedLines(word, line, lines, maxWidth, prevLine=null) {
        let firstHalf = word;
        let secondHalf = "";
        let canFitOnLine = true;

        let ctx = this.#pen.context;
        let lineWidth = ctx.measureText(line + firstHalf + "-").width;
        ...
        // if (line == prevLine)
        // {
        //     console.log(`"${secondHalf}", "${line}"`);
        //     lines.push(line[0]);

        //     line = line.slice(1, -2);
        //     secondHalf = secondHalf.slice(1, -2);
        // }

        if (ctx.measureText(secondHalf).width > maxWidth) {
            return this._addHyphenatedLines(secondHalf, "", lines, maxWidth, line);
        } else {
            return line;
        }
    }
