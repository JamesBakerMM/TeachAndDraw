...
export class Text {
    ...
    print(x, y, content, maxWidth = 0) {
        ...
        if (Number.isFinite(maxWidth) === false) {
            ErrorMsgManager.numberCheckFailed(maxWidth, "maxWidth must be a finite number!")
        }
        ...
        if (this.#tad.debug) {
            if (maxWidth <= 0) {
                Debug.drawText(this.#tad, x, y, content, maxWidth);
                return "No wrapping applied";
            } else {
                Debug.drawText(this.#tad, x, y, content, maxWidth, numOfLines);
    ...
    _addHyphenatedLines(word, line, lines, maxWidth) {
        ...
        while (this.#tad.context.measureText(line + firstHalf + "-").width > maxWidth) {
        ...
        line = secondHalf + " ";
        if (this.#tad.context.measureText(secondHalf).width > maxWidth) {
