...
export class Dropdown extends ShapedAssetEntity {
	...
	#drawOptions() {
		...
		switch (this.#pen.gui.theme) {
            case "retro":
				...
				// Account for text being too long, truncate
				const maxWidth = this.w - this.#borderWidth * 2 - 4;
				for (let i = 0; i < this.#options.length; i++) {
					const optionWidth = this.#pen.context.measureText(this.#options[i]).width
					let text = this.#options[i];
					if (optionWidth >= maxWidth) {
						text = this.#truncate(this.#options[i], maxWidth);
					}
					this.#pen.text.print(
	...
	#drawHoveredOption() {
		...
		this.#pen.shape.rectangle(
			this.x,
			this.y + directionModifier * (this.h / 2 + optionHeight * hoveringOn + adjustment),
			this.w - 4,
			optionHeight
		);
	}
    ...
    draw() {
		...
		let text = this.#options[this.#index];
		if (this.#pen.context.measureText(text).width > maxWidth) {
			text = this.#truncate(text, maxWidth);
		}
		this.#pen.colour.fill = this.textColour;
		this.#pen.text.alignment.x = "left";
		this.#pen.text.print(
