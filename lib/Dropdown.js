...
export class Dropdown extends ShapedAssetEntity {
	 ...
	 * @throws {Error} when value is not a valid colour.
	 */
	set border(value) {
		if (this.#pen.colour.isValid(value) === false) {
            throw new Error(ErrorMsgManager.colourCheckFailed(value));
		}
		this.#border = value;
	}
    ...
    #drawDecorations() {
        switch (this.#pen.gui.theme) {
            case "retro":
				...
				// White border
				this.#pen.colour.fill = this.#border; // "white";
