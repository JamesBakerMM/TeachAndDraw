...
export class Dropdown extends ShapedAssetEntity {
	 ...
	 * @property {number} focussedIndex - The index of what option is currently focussed on (by hover or keyboard controls).
     * @constructor
     */
    constructor(x, y, w, options, pen) {
        if (
	...
	*/
   	#drawFocussedOption() {
		...
		this.#pen.shape.rectangle(
			this.x,
			this.y + directionModifier * (this.h / 2 + optionHeight * this.#focussedIndex + adjustment),
			this.w - 4,
			optionHeight
		);
	}
	...
	#moveIndexUpWhen(direction) {
		if(this.#openDirection === direction) {
			if (this.#focussedIndex + 1 <= this.#options.length - 1) {
				this.#focussedIndex++;
			}
		} else {
			if (this.#focussedIndex - 1 >= 0) {
				this.#focussedIndex--;
			}
		}
	}
	/**
	 * Selects the focussed option and closes the dropdown when enter is pressed
	 */
	#handleEnterInput(){
		// Check if the draw command is issued after logic
		// Prevents issues where dropdown is non-responsive
	...
	/**
	 * Handles type and condition checking for the options parameter. Throws
	 * error if what is given is not an array, or if any items inside the array
	 * are not strings.
