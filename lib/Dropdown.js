...
export class Dropdown extends ShapedAssetEntity {
        ...
        if (newIndex > this.#options.length - 1) {
            throw Error(
                `The dropdown index is out of bounds, meaning that it goes past the end of the options array. ` +
                    `The options array length is currently ${
                        this.#options.length
                    }, meaning the maximum index that ` +
                    `can be given is ${
    ...
    set openDirection(newOpenDirection) {
        ...
        if (
            typeof newOpenDirection !== "string" ||
            (newOpenDirection !== "up" && newOpenDirection !== "down")
        ) {
            throw Error(
        ...
        if ((typeof newOpen === "boolean") === false) {
            ...
            throw Error(
                `You need to give a boolean (true or false) for the dropdown open property. ` +
                    `You gave ${newOpen}, which is a ${typeof newOpen}. ` +
    ...
    #drawButton() {
        switch (this.#tad.gui.theme) {
            case "retro":
                ...
                this.#tad.shape.rectangle(x, this.y, buttonWidth, buttonWidth);
    ...
    #drawOptions() {
        ...
        const optionsTotalHeight =
            this.#options.length * optionHeight + this.#borderWidth * 1.5;
            ...
            case "retro":
                ...
                for (let i = 0; i < this.#options.length; i++) {
                    ...
                    this.#tad.text._print(
                        ...
                        this.y +
                            directionModifier *
                                (this.h / 2 + optionHeight * i + adjustment),
                        text
                    );
                }
    ...
    isHoverOnOption(index) {
        ...
        return Dropdown.#isInRect(
            ...
            this.y +
                directionModifier *
                    (this.h / 2 + optionHeight * index + adjustment),
            this.w,
            optionHeight
        );
    }
    ...
    #drawSelected() {
        ...
        this.#tad.shape.rectangle(
            ...
            this.y +
                directionModifier *
                    (this.h / 2 + optionHeight * this.#index + adjustment),
            this.w - 4,
            optionHeight
        );
    }
    ...
    #drawFocusedOption() {
        ...
        this.#tad.shape.rectangle(
            ...
            this.y +
                directionModifier *
                    (this.h / 2 +
                        optionHeight * this.#focusedIndex +
                        adjustment),
            this.w - 4,
            optionHeight
        );
    ...
    static checkOptionsConditions(options) {
        ...
        for (let i = 0; i < options.length; i++) {
            if (typeof options[i] !== "string") {
                arrayTypeIssues += `  ❌ At index ${i}, you gave: ${
                    options[i]
                } which is a ${typeof options[i]}.\n`;
                errorFound = true;
            } else {
