...
export class TextArea extends ShapedAssetEntity {
	...
	#setupCallbacks() {
		...
		// ctl+C, ctl+X, ctl+V
	 	this.#keyctlCmd.keyCallbackModified("c", ["control"], async () => {
			...
			await this.#pen.clipboard.write(text);
		});
		...
		this.#keyctlCmd.keyCallbackModified("v", ["control"], async () => {
			const text = await this.#pen.clipboard.read();
	...
	#handleClick() {
		...
		if (this.#focused) {
			if (this.#pen.mouse.leftDown) {
				const mouseEvent = /** @type {CustomMouseEvent} */ (this.#pen.mouse.activeBuffer[0]);
	...
	#handleKeyboardInput() {
		...
		//@ts-expect-error
		this.#pen.keys.activeBuffer._forEach((/** @type {{ key: string; type: string; }} */ event) => {
