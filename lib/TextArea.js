...
class TextAreaState {
    ...
    is_same(state) {
        ...
        if (
            mismatchedCharLengths ||
            mismatchedCursorIndex ||
            mismatchedStartIndex ||
            mismatchedEndIndex
        ) {
            return false;
        }
...
export class TextArea extends ShapedAssetEntity {
    ...
    constructor(tad, x, y, w) {
        ...
        this.#keyctlText = new KeyInputController(
            tad,
            this.#keyHoldDelay,
            this.#keyRepeatDelay
        );
        this.#keyctlCmd = new KeyInputController(
            tad,
            this.#keyHoldDelay,
            this.#keyRepeatDelay
        );
    ...
    set focused(newFocused) {
        ...
        if ((typeof newFocused === "boolean") === false) {
            const stringBooleanMessage =
                newFocused === "true" || newFocused === "false"
            ...
            throw Error(
                `You need to give a boolean (true or false) for the text area focused property. ` +
                    `You gave ${newFocused}, which is a ${typeof newFocused}. ` +
                    stringBooleanMessage
            );
        }
        ...
        if (Number.isFinite(value) === false) {
            throw Error(
                ErrorMsgManager.numberCheckFailed(
                    value,
                    "character limit has to be a number!"
                )
            );
        }
        ...
        if (this.#chars.length > value) {
            throw Error(
                `You have attempted to set the characters limit to a number lower than ` +
                    `the current text content length. The current text content is ${
                        this.#chars.length
                    } ` +
    ...
    #setupCallbacks() {
        ...
        this.#keyctlText.defaultCallback = (/** @type {string} */ key) => {
            if (tad.keys.down("control") == false) this.#writeCharacter(key);
        };

        this.#keyctlCmd.keyCallback("leftarrow", () => {
            if (tad.keys.down("control")) this.#jumpCursorLeft();
            else this.#moveCursorLeft();
        });

        this.#keyctlCmd.keyCallback("rightarrow", () => {
            if (tad.keys.down("control")) this.#jumpCursorRight();
        ...
        this.#keyctlCmd.keyCallbackModified("z", ["control"], () => {
            if (this.#tad.keys.down("shift")) this.#redo();
            else this.#undo();
        });
    ...
    #handleClick() {
        ...
        if (
            this.#lastFrameDrawn !== this.#tad.time.frames - 1 &&
            this.#lastFrameDrawn !== this.#tad.time.frames
        ) {
            return;
        }
        ...
        if (this.#focused) {
            if (this.#tad.mouse.leftDown) {
                const mouseEvent = /** @type {CustomMouseEvent} */ (
                    this.#tad.mouse.activeBuffer[0]
                );
                if (mouseEvent && mouseEvent.type === "mousedown") {
                    const mouse = Vector.temp(
                        mouseEvent.adjustedX,
                        mouseEvent.adjustedY
                    );
    ...
    #getCoordFromIndex(index) {
        ...
        const charWidth = this.#tad.context.measureText(
            leftText[leftText.length - 1]
        ).width;
        const textWidth =
            this.#tad.context.measureText(leftText.join("")).width - charWidth;
        let x =
            this.x - this.w / 2 + textWidth + this.#borderWidth + this.#padding;
        let y =
            this.y -
            this.h / 2 +
            this.#tad.text.size / 2 +
            this.#borderWidth +
            this.#padding;
        return { x, y };
    }
    ...
    #mouseVector() {
        if (this.movedByCamera) {
            return this.#tad.camera.screenToWorld(
                this.#tad.mouse.x,
                this.#tad.mouse.y
            );
        }
        return Vector.temp(this.#tad.mouse.x, this.#tad.mouse.y);
    }
    update() {
        super.update();
    ...
    #drawSelection(start, end) {
        ...
        startCoords.x = Math.max(
            this.x - this.w / 2,
            Math.min(this.x + this.w / 2, startCoords.x)
        );
        endCoords.x = Math.max(
            this.x - this.w / 2,
            Math.min(this.x + this.w / 2, endCoords.x)
        );
    ...
    #calcStartEndIndices() {
        ...
        if (
            ...
            if (
                this.#tad.context.measureText(this.#chars.join("")).width >
                maxWidth
            ) {
                for (let i = 0; i < this.#chars.length; i++) {
                    const testingString = this.#chars.slice(0, i + 1).join("");
                    if (
                        this.#tad.context.measureText(testingString).width >
                        maxWidth
                    ) {
    ...
    #writeCharacter(
        char,
        maxWidth = this.w - 2 * (this.#borderWidth - this.#padding)
    ) {
    ...
    #handleKeyboardInput() {
        ...
        this.#tad.keys.activeBuffer.forEach(
			//@ts-expect-error
            (/** @type {{ key: string; type: string; }} */ event) => {
                if (
                    event.key.length === 1 &&
                    event.type === "keydown" &&
                    allowTyping
    ...
    #moveWindowThroughDeletion() {
        ...
        const maxWidth = this.w - this.#borderWidth * 2 - this.#padding * 2;
        const testingString = this.#chars
            .slice(this.#startIndex - 1, this.#endIndex + 1)
            .join("");
        const textWidth = this.#tad.context.measureText(testingString).width;
        if (textWidth <= maxWidth) {
            // Move start index if char doesn't fit
     ...
     */
    #jumpCursorLeft() {
        this.#cursorIndex = TextAreaUtil.findNotCharacter(
            " ",
            this.#cursorIndex - 1,
            -1,
            this.#chars
        );
        this.#cursorIndex = TextAreaUtil.findCharacter(
            " ",
            this.#cursorIndex,
            -1,
            this.#chars
    ...
    #cursorIndexBound() {
        this.#cursorIndex = Math.max(
            0,
            Math.min(this.#cursorIndex, this.#chars.length)
        );
    }
    /**
...
class KeyInputController {
    ...
    #validateKey(key) {
        if ((typeof key === "string") === false) {
            throw Error(
                `key must be a string. You supplied a ${typeof key} (${key}).`
            );
        }
    }
    /**
