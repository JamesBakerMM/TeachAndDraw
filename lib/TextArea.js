		...
		if (this.#cursorVisible) {
			...
			for (let i = 0; i < this.#lines.length; i++) {
				...
				if (count <= 0) { //This current line
					...
					// ok so the cursor index doesnt match up with the lines arrays because spaces were taken away.
					// so thats a little bit of an issue, but otherwise it IS working!
					// if i instead use line and index, the math ive got here won't work, but maybe it'll be easier?
					// if i do need the normal index later i can probably calculate it, but it still wouldnt match up with the chars
					// so there are two options: somehow fix the word wrap to not delete spaces OR
					// not use the chars array at all, and not use cursorIndex. Is it better to have
				}
			}
