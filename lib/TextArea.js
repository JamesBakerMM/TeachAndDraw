...
export class TextArea extends ShapedAssetEntity {
	...
	#moveCursorWithMouse() {
		...
		const newCursorIndex = this.#getIndexFromCoords(mouseX);
	...
	#handleKeyboardInput() {
		...
		// this.#readFromClipboard().then((text => "idk"));
		// this.#writeToClipboard("hi");
		// ASKS FOR PERMISSION. how to handle?
		// console.log(navigator.userAgent);
		// ok we have meta down /control down and then up. While down, and one of the options are clicked
		// then its a command. Could use "down"?
		// const mac = navigator.userAgent.toLowerCase().includes("mac");
		// if (this.#pen.keys.activeBuffer[0]) {

		// 	console.log(this.#pen.keys.activeBuffer[0].type);
		// }
		// if (this.#pen.keys.down("meta") || (!mac && this.#pen.keys.down("control"))) {
		// 	const keys = this.#pen.keys.activeBuffer._map((event) => {
		...
		// 	if (keys.includes("v")) { //Paste
		// 		console.log("paste");
		// 	} else if (keys.includes("x")) { //Cut
		// 		console.log("cut");
		// 	} else if (keys.includes("c")) { //Copy
		// 		console.log("copy");
		// 	} else if (keys.includes("a")) { //Select All
		// 		console.log("select all");
		// 	}
		// 	return; // Don't allow typing, exit function
		// }
		...
		const maxWidth = this.w - this.#borderWidth * 2 - this.#padding * 2;
		// console.log(this.#pen.context.measureText(this.#chars.join("")).width);
		// console.log(this.#chars.join(""));
		console.log(this.#pen.context.font)
	...
	// 		return clipText;
	// 	} catch (err) {
	// 		console.error("Something has gone wrong with accessing the clipboard.", err);
	// 	}
	// }
