...
export class TextArea extends ShapedAssetEntity {
    ...
    constructor(pen, x, y, w) {
		...
		for (let i=0; i<256; i++)
		{
			this.#keyFlags[String.fromCharCode(i)] = true;
		}
	...
	#keyReady( key )
	{
		return this.#pen.keys.down(key) && this.#keyFlags[key] == true;
	}
	...
	#handleKeyboardShiftModifier()
	{
		if (this.#pen.keys.down("shift"))
		{
			if (this.tmpIdx == -1)
			{
				this.tmpIdx = this.#cursorIndex;
			}
			
			this.#selStartIndex = this.tmpIdx;
			this.#selEndIndex = this.#cursorIndex;
		}
	...
	#pasteSelection() {
		...
		const clipboard = this.#pen.text.clipboard;
		const left  = Math.min(this.#selStartIndex, this.#selEndIndex);
		const right = Math.max(this.#selStartIndex, this.#selEndIndex);
		...
		for (let c of clipboard)
		{
			this.#chars.splice(this.#cursorIndex, 0, c);
			this.#cursorIndex += 1;
		}

		this.#cursorIndex   = left + clipboard.length;
		this.#selStartIndex = left;
		this.#selEndIndex   = this.#cursorIndex;
	}
	...
	#insertCharacter( char )
	{
		...
		this.#chars.splice(left, right-left, char);
	...
	#removeCharacter()
	{
		...
		const left  = Math.min(this.#selStartIndex, Math.min(this.#cursorIndex, this.#selEndIndex));
		const right = Math.max(this.#selStartIndex, Math.max(this.#cursorIndex, this.#selEndIndex));

		this.#chars.splice(left-1, right-left+1);
