...
export class TextArea extends ShapedAssetEntity {
	...
	#drawCursor() {
		...
		if (this.#cursorVisible) {
			...
			this.#pen.shape.rectangle(this.x, this.y, 1, this.#pen.text.size);
	...
	#drawText() {
		...
		const padding = 3;
		const maxWidth = this.w - this.#borderWidth * 2 - padding * 2;
		...
		for (let i = 0; i < lines.length; i++) {
            ...
            this.#pen.context.fillText(lines[i], this.x - (this.w - this.#borderWidth * 2) / 2 + padding, this.y - (this.h - this.#borderWidth * 2) / 2 + padding + direction);
        }
	...
	#handleKeyboardInput() {
		...
		if (
			this.#pen.keys.activeBuffer.length > 0 && 
			this.#pen.keys.activeBuffer[0].key.length === 1
		) {
			this.#chars.splice(this.#cursorIndex, 0, this.#pen.keys.activeBuffer[0].key);
		}
