...
export class TextArea extends ShapedAssetEntity {
	...
	//  */
	#drawSelection() {
		const start = Math.max(this.#selStartIndex, this.#startIndex);
		const end   = Math.max(this.#selEndIndex, this.#endIndex);
	...
	#writeCharacter( char, maxWidth = this.w - 2*(this.#borderWidth-this.#padding)) {
		...
		console.log(this.#selStartIndex, this.#selEndIndex);
	...
	#writeBackspace( maxWidth = this.w - 2*(this.#borderWidth-this.#padding) )
	{
		...
		console.log(this.#selStartIndex);
		console.log(this.#selEndIndex);
