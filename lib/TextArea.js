...
export class TextArea extends ShapedAssetEntity {
	 ...
	 */
	#moveCursorWithMouse() {
		const leftSide = this.x - this.w / 2;
		const mouseDistFromLeft = this.#pen.mouse.x - leftSide;
		let prevDist;
		for (let i = this.#startIndex; i <= this.#endIndex + 1; i++) {
			const tempChars = [...this.#chars, ""];
			const leftText = tempChars.slice(this.#startIndex, i);
			...
			let x = textWidth + this.#borderWidth + this.#padding;

			if (x >= mouseDistFromLeft) {
				const prevDiff = Math.abs(prevDist - mouseDistFromLeft);
				const currDiff = Math.abs(x - mouseDistFromLeft);
				if (currDiff < prevDiff) {
					this.#cursorIndex = i - 1;
				} else {
					this.#cursorIndex = i - 2;
				}
				break;
	...
	#drawSelection(start, end) {
		// Start location
		let startCoords = this.#getCoordFromIndex(start);

		// End location
		let endCoords = this.#getCoordFromIndex(end);
	...
	#drawCursor() {
		...
		if (this.#cursorVisible) {
			const tempChars = [...this.#chars, ""];
			const leftText = tempChars.slice(this.#startIndex, this.#cursorIndex + 1);
			const charWidth = this.#pen.context.measureText(leftText[leftText.length - 1]).width;
			const textWidth = this.#pen.context.measureText(leftText.join("")).width - charWidth;
			let x = textWidth + this.#borderWidth + this.#padding;
			let y = this.#pen.text.size / 2 + this.#borderWidth + this.#padding;
