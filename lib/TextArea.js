...
class TextAreaState {
	 ...
	 * @param {number} selDrawStartIndex
	 * @param {number} selDrawEndIndex
	 * @param {any} startIndex
	 * @param {any} endIndex
	 */
	constructor( chars, cursorIndex, keySelectIndex, selDrawStartIndex, selDrawEndIndex, startIndex,
				 endIndex )
	{
		this.chars = chars.slice();
		this.cursorIndex = cursorIndex;
		this.keySelectIndex = keySelectIndex;
		this.selDrawStartIndex = selDrawStartIndex;
		this.selDrawEndIndex = selDrawEndIndex;
	...
	is_same( state )
	{
		...
		const mismatchedStartIndex = this.selDrawStartIndex != state.selDrawStartIndex;
		const mismatchedEndIndex = this.selDrawEndIndex != state.selDrawEndIndex;
...
export class TextArea extends ShapedAssetEntity {
	 ...
	 * @property {number} selDrawStartIndex - The selection start index, corresponging to chars array, for what is currently selected by the mouse. (Not necessarily lower than end)
	 * @property {number} selDrawEndIndex - The selection end index, corresponging to chars array, for what is currently selected by the mouse. (Not necessarily higher than start)
    ...
    constructor(tad, x, y, w) {
		...
		this.#endIndex = 0;
		this.#selDrawStartIndex = 0;
		this.#selDrawEndIndex = 0;
		this.#selClipStartIdx = 0;
		this.#selClipEndIdx = 0;
	...
	#setupCallbacks() {
		...
		this.#keyctlCmd.keyCallbackModified("a", ["control"], () => {
			this.#selClipStartIdx = 0;
			this.#selClipEndIdx   = this.#chars.length;
			this.#selDrawStartIndex = this.#startIndex;
			this.#selDrawEndIndex   = this.#endIndex+1;
			this.#cursorIndex = this.#selDrawEndIndex;
		});
		...
		this.#keyctlCmd.keyCallback("End", () => {
			...
			this.#cursorIndex=L; 
			this.#selDrawStartIndex=L; 
			this.#selDrawEndIndex=L;
		});
		...
		this.#keyctlCmd.keyCallback("Backspace", () => {
			if (this.#tad.keys.down("control") == false) {
				...
				this.#selDrawStartIndex = left;
				this.#selDrawEndIndex   = right;
				this.#writeBackspace();
				this.#pushState();
			}
		});
		...
		if (this.#focused) {
			if (this.#tad.mouse.leftDown) {
				...
				if (mouseEvent && mouseEvent.type === "mousedown") {
					const newIndex = this.#getIndexFromCoords(mouseEvent.adjustedX);
					if (newIndex !== undefined) {
						this.#selDrawStartIndex = newIndex;
					}
				}

				const currEndIndex = this.#getIndexFromCoords(this.#tad.mouse.x);
				if (currEndIndex !== undefined) {
					this.#selDrawEndIndex = currEndIndex;
	...
	#handleOutsideClick() {
		...
		if (!this.#hovered) {
			...
			if (mouseEvent && mouseEvent.type === "mousedown") {
				this.#focused = false;
				this.#selDrawStartIndex = this.#cursorIndex;
				this.#selDrawEndIndex = this.#cursorIndex;
			}
		}
	}
	...
	#pushState() {
		const state = new TextAreaState(
			...
			this.#selDrawStartIndex, this.#selDrawEndIndex, this.#startIndex, this.#endIndex
		);
	...
	#loadState( state ) {
		...
		this.#selDrawStartIndex = state.selDrawStartIndex;
		this.#selDrawEndIndex = state.selDrawEndIndex;
	...
	#handleKeyboardNavigation() {
		if (this.#tad.keys.down("shift")) {
			if (this.#keySelectIndex == -1) {
				this.#keySelectIndex = this.#selDrawStartIndex;
			}
			this.#selDrawStartIndex = this.#keySelectIndex;
			this.#selDrawEndIndex   = this.#cursorIndex;
		} else {
			this.#keySelectIndex = -1;
		}
    ...
    update() {
		...
		const mouse = Vector.temp(this.#tad.mouse.x, this.#tad.mouse.y);
		const rect  = Vector.temp(this.x, this.y);
		mouse.subtract(rect);
		mouse.rotate(-this.rotation);
		mouse.add(rect);
		this.#tad.shape.oval(mouse.x, mouse.y, 20, 20);
		this.#hovered = TextArea.#isInRect(
			mouse.x, mouse.y,
			rect.x,  rect.y, this.w, this.h
		);
	 ...
	 */
	#copySelection() {
		const left  = Math.min(this.#selDrawStartIndex, this.#selDrawEndIndex);
		const right = Math.max(this.#selDrawStartIndex, this.#selDrawEndIndex);
		let str = "";
	...
	//  */
	#drawSelection() {
		const start = Math.max(this.#selDrawStartIndex, this.#startIndex);
		const end   = Math.max(this.#selDrawEndIndex, this.#endIndex);
	...
	#writeCharacter( char, maxWidth = this.w - 2*(this.#borderWidth-this.#padding)) {
		...
		let left  = Math.min(this.#selDrawStartIndex, this.#selDrawEndIndex);
		let right = Math.max(this.#selDrawStartIndex, this.#selDrawEndIndex);
		let delta = right-left;

		console.log(this.#selDrawStartIndex, this.#selDrawEndIndex);

		this.#chars.splice(left, delta, char);

		this.#keySelectIndex = left+1;
		this.#selDrawStartIndex  = left+1;
		this.#selDrawEndIndex    = left+1;
	...
	#writeBackspace( maxWidth = this.w - 2*(this.#borderWidth-this.#padding) )
	{
		...
		console.log(this.#selDrawStartIndex);
		console.log(this.#selDrawEndIndex);
