export class CollisionUtilities {
    ...
    static lineIntersectsLine(x1, y1, x2, y2, x3, y3, x4, y4) { 
        ...
        if (m1 == m2 || Number.isNaN(c1) || Number.isNaN(c2)) { //return null if lines are parallel
            return null;
        }
        let xIntersection = 0;
        let yIntersection = 0;
        if (!Number.isFinite(m1) || Number.isNaN(m1)) {
            xIntersection = x1;
            yIntersection = (m2 * xIntersection) + c2;
        } else if (!Number.isFinite(m2) || Number.isNaN(m2)) {
            xIntersection = x3;
