...
export class CollisionUtilities {
    ...
    static angleBetweenVectors(v1, v2) {
        let value = CollisionUtilities.dotProduct(v1, v2) / (CollisionUtilities.distance(v1.x, v1.y) * CollisionUtilities.distance(v2.x, v2.y));
        if (value < -1) {
            value = -1;
        }
        if (value > 1) {
        ...
        return CollisionUtilities.convertToDegrees(Math.acos(value));
    }
