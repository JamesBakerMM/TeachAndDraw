...
export class Entity {
     ...
     * @property {number} ageless - Whether the object ages/decrements it's lifespan.
     * @property {number} lifespan - Lifespan of the entity. Infinity by default.
     * @constructor
     */
    constructor(pen, x, y) {
    ...
    set ageless(value) {
        if (typeof value !== "boolean") {
            throw new Error(
                `ageless must be a boolean! You supplied a ${typeof value}.`
            );
        }
        this.#ageless = value;
    }
    ...
    /**
     * Internal update method intended to be overridden by derived class.
     * Separate logic from rendering by updating the derived class using this method.
     */
    internalUpdate() {
    ...
    /**
     * Public-facing update method, not intended to be overridden by derived classes.
     */
    update() {
        if (this.#pen.frameCount === this.#frameLastUpdated) {
            return;
        }
        this.internalUpdate();
        this.#frameLastUpdated = this.#pen.frameCount;

        if (this.ageless) {
            return;
        }
        if (this.#lifespan > 0) {
            this.#lifespan -= (this.#pen.time.msElapsed / 1000.0);
        } else {
            this.remove();
        }
    }
