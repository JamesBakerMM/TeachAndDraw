...
export class Collider extends ShapedAssetEntity {
    ...
    collides(otherCollider) {
        ...
        if (this.overlaps(otherCollider)) {
            ...
            if (Math.abs(absoluteXDepth - absoluteYDepth) < 0.1) {
            ...
            } else if (absoluteXDepth > absoluteYDepth) {
                if (fromRight) {
                    this.x = otherCollider.right + halfWidth;
                    console.log("TO TJHE RIGHT");
                } else {
                    this.x = otherCollider.left - halfWidth;
                }
                this.velocity.x = -this.velocity.x * this.bounciness;
    ...
    calculateElasticCollision(velocityA, massA, velocityB, massB) {
        const newVelocityA =
            ((massA - massB) * velocityA + 2 * massB * velocityB) /
            (massA + massB);
        const newVelocityB =
            ((massB - massA) * velocityB + 2 * massA * velocityA) /
            (massA + massB);
