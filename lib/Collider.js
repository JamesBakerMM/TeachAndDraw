...
export class Collider extends ShapedAssetEntity {
    ...
    updateCollision() {
        ...
        if (this.shape === "circle") {
            this.collisionShape = new CollisionCircle(0, 0, this.radius);
            ...
            if (stepsTotal < 2) {
                ...
                this.#stepsX = this.#stepsTotal;
                this.#stepsY = this.#stepsTotal;
                ...
                const half_height = this.radius;

                let p1 = {x: center.x - half_width, y: this.y - half_height};
                let p2 = {x: center.x + half_width, y: this.y - half_height};
                let p3 = {x: center.x - half_width, y: this.y + half_height};
                let p4 = {x: center.x + half_width, y: this.y + half_height};

                p1 = CollisionUtilities.rotatePointAroundPoint(p1.x, p1.y, this.x, this.y, rotation);
                p2 = CollisionUtilities.rotatePointAroundPoint(p2.x, p2.y, this.x, this.y, rotation);
                p3 = CollisionUtilities.rotatePointAroundPoint(p3.x, p3.y, this.x, this.y, rotation);
                p4 = CollisionUtilities.rotatePointAroundPoint(p4.x, p4.y, this.x, this.y, rotation);

                this.collisionShape = new CollisionPolygon(0, 0, [p1, p2, p3, p4]);
    ...
    updateBounds() {
        if (this.collisionShape != null) {
            this.#minX = this.x + this.collisionShape.minX;
            this.#maxX = this.x + this.collisionShape.maxX;
            this.#minY = this.y + this.collisionShape.minY;
            this.#maxY = this.y + this.collisionShape.maxY;
        } else {
