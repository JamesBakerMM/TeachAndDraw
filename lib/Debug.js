...
export class Debug {
    ...
    static constructMemoryPane(){
        if(performance && performance.memory){
            return `<details>
                ...
                <ul>
                    <li>Total HeapSize:${performance.memory.totalJSHeapSize} </li>
                    <li>Used HeapSize:${performance.memory.usedJSHeapSize} </li>
                    <li>HeapSize Limit:${performance.memory.jsHeapSizeLimit} </li>
                </ul>
            </details>`
        }
    ...
    static constructGroupPane(){
        let html=`<details>
            <summary>Group: ${Group.all.length} groups</summary>
            <hr>
            <ul id="groups">`
        for(let group of Group.all){
            html+=`<li>Group(${group.length}):${group.type}</li>`
        }
        html+="</ul></details>";
        return html
    }
    ...
    static async constructInfoPane(pen) {
        ...
        let html = `                    
            Canvas w:${pen.w} h:${pen.h}
            <hr>
                <details>
                    <summary>Pen Details</summary>

                    <div><strong>Paused:</strong> ${pen.paused}</div>
                    <div><strong>Debug Mode:</strong>${pen.debug}</div>
                </details>
                    <hr>

                    <details>
                        <summary>Fps: ${pen.frameCount} (${pen.fps})</summary>
                        <div><strong>Target FPS:</strong> ${pen.fps}</div>
                        <div><strong>Current Frame:</strong> ${pen.frameCount}</div>
                        <div><strong>Actual Fps:</strong>Nope</div>
                    </details>
                    <hr>
                    <details>
                    <summary>Draw State</summary>
                        <h4>Colour:</h4>
                        <div><strong>Fill:</strong> ${curState.colour.fill} </div>
                        <div><strong>Stroke:</strong> ${curState.colour.stroke} </div>
                        <hr>
                        
                        <h4>Shape:</h4>
                        <div><strong>Stroke Width:</strong> ${pen.shape.strokeWidth}</div>
                        <div><strong>X Alignment:</strong> ${pen.shape.xAlignment}</div>
                        <div><strong>Y Alignment:</strong> ${pen.shape.yAlignment}</div>
                        <!-- Add other Shape properties as needed -->
 
                        <h4>Text:</h4>
                        <div><strong>Alignment:</strong> ${pen.text.alignment}</div>
                        <div><strong>Baseline:</strong> ${pen.text.baseline}</div>
                        <div><strong>Size:</strong> ${pen.text.size}</div>
                    </details>                       
                    <hr>
        `;
