...
export class Debug {
        ...
        let html = `                    
            ...
            <hr>
                    ...
                    <br>
                        ...
                        <strong>Alignment:</strong> ${pen.text.alignment.x} | 
                        <strong>Baseline:</strong> ${pen.text.alignment.y}
                    <hr>
        `;
    ...
    static drawCollider(pen, collider) {
        ...
        pen.colour.stroke = Paint.aqua.pale[0];
        ...
        // pen.shape.rectangle(collider.x, collider.y, collider.w, collider.h);
        ...
        // pen.shape.oval(collider.x, collider.y, 10);
        // pen.colour.fill = COLOURS.BLACK;
        // pen.text.draw(collider.x, collider.y, `${collider.id}`);
        // pen.colour.fill = COLOURS.IMG_ORANGE;
    ...
    static drawMouse(pen, mouse) {
        ...
        if (mouse.isPressed) {
            ...
            pen.colour.fill = "grey";
            if(mouse.left){
                pen.colour.fill="white";
            }
            
            pen.shape.xAlignment="right";
    ...
    static cross(x,y){
        pen.colour.stroke="rgba(255,255,0,0.3)";
    ...
    static drawGrid(pen) {
        ...
        for (let i = 0; i < pen.w / INCREMENT; i++) {
            ...
            pen.text.draw(i * INCREMENT, INCREMENT-5, i * INCREMENT);
        }
        ...
        for (let y = 0; y < pen.h / INCREMENT; y++) {
            if(y * INCREMENT!==50){ //prevent overlapping tag
                ...
                pen.colour.fill = DULL_GREEN;
                pen.text.draw(INCREMENT, INCREMENT * y-5, y * INCREMENT);
            }
        }
    }
