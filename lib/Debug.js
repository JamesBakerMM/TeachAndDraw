...
export class Debug {
    ...
    static drawGrid(tad) {
        ...
        for (let i = 0; i < tad.w / INCREMENT; i++) {
            ...
            tad.text.print(x, INCREMENT - 5, `${x}`);

            // vertical line at x
            tad.shape.line(
                x,
                INCREMENT,      // or 0 if you want it behind the label too
                x,
                tad.h
            );
        }
        ...
        for (let y = 0; y < tad.h / INCREMENT; y++) {
            const gy = y * INCREMENT;

            if (gy !== 50) { // prevent overlapping tag
                ...
                tad.shape.colour = BRIGHT_GREEN;
                tad.text.alignment.x = "left"
                tad.text.print(INCREMENT, gy - 5, `${gy}`);
