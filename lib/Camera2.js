...
export class Camera2 {
    ...
    set zoom(zoom) {
        if (Number.isFinite(zoom) === false) {
            throw Error(
                ErrorMsgManager.numberCheckFailed(zoom, "value has to be a number!")
            );
        }
    
        if (zoom < 1.0) {
            throw Error(
                ErrorMsgManager.numberCheckFailed(zoom, "zoom must be greater than zero!")
            );
        }
    
        // zoom = this.#tad.math.clamp(zoom, 1, 10);
