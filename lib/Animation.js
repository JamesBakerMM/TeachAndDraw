...
export class MovingStamp extends Stamp {
    ...
    clone() {
        const nuMovingStamp = new MovingStamp(this.#pen, this.x, this.y, ...this.#frames);
    ...
    _draw(x = this.x, y = this.y, rotation = this.rotation) {
        ...
        this.#pen.context.rotate(this.#pen.math.degreeToRadian(rotation % 360));
        this.#pen.context.translate(this.offset.x, this.offset.y);
        this.#pen.context.scale(scaleX, scaleY);

        const frame = this.#frames[this.frame];
        //@ts-expect-error
        this.#pen.context.drawImage(frame.raw, -frame.w/2, -frame.h/2, frame.w, frame.h);
        ...
        // This is done after drawing otherwise frames[1] would be the first frame drawn.
        this.#progressFrame();
    }
     ...
     */
    #progressFrame() {
        this.#timer += this.#pen.time.msElapsed;
