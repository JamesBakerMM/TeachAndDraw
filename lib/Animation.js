...
export class MovingStamp extends Stamp {
    ...
    clone() {
        const nuMovingStamp = new MovingStamp(
            this.#tad,
            this.x,
            this.y,
            ...this.#frames
    ...
    update() {
        ...
        if (this.#tad.time.frames - this.#lastFrameDrawn <= 1) {
            this.#progressFrame();
        }
    }
    ...
    /**
     * Set the width of every image in this animation to the given value.
     * @param {Number} value
     */
    set w(value) {
        if (!Number.isFinite(value)) {
            throw new TypeError(ErrorMsgManager.numberCheckFailed(value));
        }
    ...
    /**
     * Set the height of every image in this animation to the given value.
     * @param {Number} value
     */
    set h(value) {
        if (!Number.isFinite(value)) {
    ...
    get frame() {
        let idx = Math.floor(
            this.#frames.length * (this.#timer / this.#duration)
        );
