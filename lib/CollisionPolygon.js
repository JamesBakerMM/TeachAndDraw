...
export class CollisionPolygon extends CollisionShape {
    ...
    doesCircleIntersect(circle) {

        //check if the circle center is within the polygon
        const result = this.isPointWithin(circle.x, circle.y);
        if (result) {
            return true;
        }
        ...
        for (let i = 0; i < this.#edges.length; i++) {
            const result = this.#edges[i].doesCircleIntersect(circle);
            if (result) {
                return true;
            }
        }
    ...
    doesPolygonIntersect(anotherPolygon) {
        ...
        for (let i = 0; i < anotherPolygon.#points.length; i++) {
            ...
            const result = this.isPointWithin(point.x, point.y)
            if (result) {
                return true;
            }
        }
