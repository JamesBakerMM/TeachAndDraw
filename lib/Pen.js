...
${Video.loading.title} | ${Video.loading.url}
    ...
    loadCustomFont(name, filepath) {
        ...
        let acceptedFiletypesMsg = "";
        for (const fileType of fileTypes) {
            acceptedFiletypesMsg += `- ${fileType}\n`;
        }
        if (fileTypes.includes(filepathEnding) === false) {
            throw new Error(
                `Filepath is not an accepted file type for custom fonts! You gave the path ${filepath}, which is a file ending in ${filepathEnding}.\n` +
                `The accepted file endings are:\n` +
                acceptedFiletypesMsg
            );
        }
        //no online links, local filepaths only
        // ttf, otf, woff or woff2 only
        // How to test that the local filepath works, when its just passed to the url function?
        /**
         * 1. Check that the url is valid/a font type. Check name is string too
         * 2. Should I have some checking to make sure that the name they gave doesn't already exist or something?
         *      Like what if they try and overwrite Helvetica or something? Maybe that won't matter using a customFont property
         * 3. Create the "@font-family" css with the variables. local() and url()?
         * 4. Return the name they give, so that it can be used like const font = $.text.loadCustomFont("name", url)
         *      and then like $.text.customFont = font;
         */

        //Ok im thinking that we could have a computer property customFont, that does checks that normal font doesn't.
        //And in the end, it just adds the font to the normal font property. Is there a way to make sure that
        //custom fonts aren't being used with the normal font property? Bc then they circumvent all the nice errors
        //But if i just CHECK for the custom fonts in the normal font, i can only find them when they're correct, defeating
        //the purpose of the fancy errors. And we discussed not adding the warnings all the time
    }
