...
${Video.loading.title} | ${Video.loading.url}
    ...
    #handleError( error ) {
        if (error instanceof EvalError) {
            throw new Error(`Caught EvalError\nName: "${error.name}"\nMessage: "${error.message}"`);
        } else if (error instanceof RangeError) {
            throw new Error(`Caught RangeError\nName: "${error.name}"\nMessage: "${error.message}"`);
        } else if (error instanceof ReferenceError) {
            throw new Error(`Caught ReferenceError\nName: "${error.name}"\nMessage: "${error.message}"`);
        } else if (error instanceof SyntaxError) {
            throw new Error(`Caught SyntaxError\nName: "${error.name}"\nMessage: "${error.message}"`);
        } else if (error instanceof TypeError) {
            throw new Error(`Caught TypeError\nName: "${error.name}"\nMessage: "${error.message}"`);
        } else if (error instanceof URIError) {
            throw new Error(`Caught URIError\nName: "${error.name}"\nMessage: "${error.message}"`);
        } else if (error instanceof AggregateError) {
            throw new Error(`Caught AggregateError\nName: "${error.name}"\nMessage: "${error.message}"`);
        } else if (error instanceof TypeError) {
            throw new Error(`Caught TypeError\nName: "${error.name}"\nMessage: "${error.message}"`);
        } else {
    ...
    drawCanvas() {
        ...
        if (!document.hidden) {
            ...
            if (this.#assets.isLoaded() === false) {
                ...
                /*
                    Currently calculating dt here even though TimeManager does so. In this case,
                    msThen is only updated once the target dt is reached, while TimeManager.msThen
                    is updated every frame regardless of dt. Changing TimeManager would alter the
                    behaviour of various entities and physics calculations.
                */
                const msNow = window.performance.now();
                const msElapsed = msNow - this.msThen;
                const target = 1000 / this.time.fps;

                if (msElapsed >= target) {
                    this.clearCanvas();
                    this.upkeep();

                    const msExcess = msElapsed % target;
                    this.msThen = msNow - msExcess;
