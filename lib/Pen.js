...
${Video.loading.title} | ${Video.loading.url}
     ...
     * @example
     * const dropdown = $.makeDropdown(200, 100, 100, 30, ["Option 1", "Option 2", "Option 3"]);
     * dropdown.draw();
     ...
     * @param {string[]} options - The list of options contained in the dropdown. Must be at least 2, and be strings.
     * @returns {Dropdown} The newly created Dropdown object.
     * @throws {Error} If x, y, w, h are not finite numbers or if options is not an array of strings with > 1 option.
     */
    makeDropdown(x, y, w, h, options) {
        if (Number.isFinite(x) === false) {
            throw new Error(
                ErrorMsgManager.numberCheckFailed(x, "x has to be a number!")
            );
        ...
        if (options.length < 2) {
            throw Error(
                `You need to give at least 2 options in the options array. You gave ${options.length}.`
            )
        }
        let arrayTypeIssues = ``;
        ...
        return new Button(x, y, w, h, options, this);
    }
    /**
