...
export class Pen {
    ...
    loadAnimation(x, y, ...filepaths) {
        if (Number.isFinite(x) === false || Number.isFinite(y) === false) {
            throw Error(
                `x and y have to be numbers you gave x:${x}|${typeof x} y:${y}|${typeof y}`
            );
        }
    ...
    loadImage(x, y, filepath) {
        if (Number.isFinite(x) === false || Number.isFinite(y) === false) {
            throw new Error(`x and y need to be numbers you gave:\nx:${x}:${typeof x},\ny:${y}:${typeof y}`);
        }
    ...
    drawCanvas() {
        ...
        if (elapsed > this.fpsInterval && !document.hidden) {
            ...
            if (this.#assets.isLoaded() === false) {
                ...
                this.kb.draw();
                this.mouse.draw();
                this.#collisions.handleCollisions(Collider.all); //pass this the array of colliders that called collide
                Group.cleanup();
            }
        }
