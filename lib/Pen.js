...
${Video.loading.title} | ${Video.loading.url}
    ...
    loadImage(x, y, filepath) {
        ...
        if (typeof filepath !== "string") {
            throw new Error(`Filepath is not a string! You gave: ${filepath}|${typeof filepath}.`);
        }
        if(this.frameCount>=0){
            throw new Error(
                "You can't load after draw has started, load assets before hand and store the result in a variable, object property or group!"
    ...
    makeCircleCollider(x, y, w, h) {
        ...
        return new Collider(this, x, y, w, h, "circle");
    }
    /**
     * 
     * @param {function} userDrawFunc 
