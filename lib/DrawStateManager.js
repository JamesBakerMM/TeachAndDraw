...
export class DrawStateManager {
    ...
    save() {
        ...
        if (this.stack.length > 100) {
            console.warn(
                "over 100 states are saved currently this could cause performance impacts! check how often you are calling state.save() before you call state.load()"
            );
        }
    }
     ...
     * Intended to be used at the start of each frame.
     */
    reset() {
        if (this.stack.length > 0) {
            console.warn(
                "you have states saved from the previous frame! you likely forgot to call a state.load() somewhere :)"
            );
        }
...
export class DrawState {
    constructor(pen) {
        ...
        this.shape = {
            ...
            alignment: {
                x: pen.shape.alignment.x,
                y: pen.shape.alignment.y,
            },
            strokeDash: 0,
        };

        /**
         * Text properties of the current state.
         *
         */
        this.text = {
            alignment: {
                x: pen.text.alignment.x,
                y: pen.text.alignment.y,
            },
            bold:pen.text.bold,
            italic:pen.text.italic,
            size: 16,
            font: pen.text.font,
        };
    }
