...
export class ErrorMsgManager {
    ...
    static loadFileFailed(filepath, type, msg = "") {
        ...
        switch (type) {
            case `img`:
                ...
                if (
                    filetypeFromPath &&
                    validImageTypes.has(filetypeFromPath.toLowerCase())===false
                ) {
                    msg += `${tab(1)}◼ img filetype (${filetypeFromPath}) is not a valid type for loadImage!\n ${tab(2)}>It needs to be one of the following types: ${[...validImageTypes].join(", ")}\n`;
                }
                break;
            ...
            case `json`:
                if (filetypeFromPath.toLowerCase()!=="json") {
                    msg += `${tab(1)}◼ invalid filetype ("${filetypeFromPath}") given for .json file!\n`;
                }
            ...
            case `sound`:
                ...
                if (
                    filetypeFromPath &&
                    validSoundTypes.has(filetypeFromPath.toLowerCase()) === false
                ) {
                    msg += `${tab(1)}◼ sound filetype (${filetypeFromPath}) is not a valid type for loadSound!\n${tab(2)}>It needs to be one of the following types: ${[...validSoundTypes].join(", ")}\n`;
                }
                break;
            default:
        }
        ...
        const filenameSymbols = this.containsSymbols(filename.toLowerCase());

        if(filenameSymbols.length>0){
            msg+=`${tab(1)}◼ filename is using complex symbols, we recommend removing these symbols: ${filenameSymbols.join(',')} from the name\n`;
        }
    ...
    static containsSymbols(str) {
        const matches = str.matchAll(/[^a-zA-Z0-9/._\s-]/g) || [];
