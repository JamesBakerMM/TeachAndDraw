...
export class ErrorMsgManager {
    ...
    static cleanErrorStack(stack) {
        ...
        newStack = newStack.replace("Object.update", "function update()")
        return newStack;
    }
    ...
    static handleTadTypeError(error) {
        ...
        for (const module of ErrorMsgManager.modules) {
            ...
            if (looksUndefined) {
                ...
                if (extracted_field) {
                    ...
                    hint += `\nCheck the spelling. Valid methods: ${module.methods.join(", ") || "(none)"
                        }`;
                    if (module.properties?.length) {
        ...
        const propertyParentIsUndefined = msg.includes('Cannot read properties of undefined')
        if (propertyParentIsUndefined) {
            const chromeMatch = msg.match(/Cannot read properties of undefined \(reading '(.+)'\)/);
            const readProp = chromeMatch ? chromeMatch[1] : null;
            //-------------
            const undefinedParentSubtype = ErrorMsgManager.determineUndefinedParent(readProp);
            let userMsg = "\nYou tried to use a property on something that is actually 'undefined'.";
            let type = "undefinedParent";
            //-------------
            if (undefinedParentSubtype === "arrayIndex") {
                type = "undefinedParentGroupIndex";
                userMsg = "\nYou tried to index[" + readProp + "] into something that is 'undefined'." +
                    "\nThis usually means a nested loop went too far, or you tried to index into something that is not in fact an Group." +
                    "\nDid you try to nest Groups? Eg 'grid[row][col]', if 'grid[row]' is undefined, you would get this error.";
            } else if (undefinedParentSubtype === "arrayLength") {
                type = "undefinedParentArrayLength";
                userMsg = "\nYou tried to read '.length' from something that is 'undefined'." +
                    "\nThis usually means:\n" +
                    "- You forgot to create or fill the group before using it, or\n" +
                    "- A loop went out of range and you're doing 'exampleGroup[i].length' where 'exampleGroup[i]' is undefined.";

            } else if (undefinedParentSubtype === "objectProp") {
                type = "undefinedParentObject"
                userMsg = `\n.${readProp} is being read from something that is undefined or null.` +
                    "\nThis usually means a variable wasn't initialised, or was set to undefined/null before you used it." +
                    "\nDouble-check where that object is created or assigned.";
            }
            return {
                isTad: true,
                error,
        ...
        const cannotSetUndefined = msg.startsWith("Cannot set properties of undefined");
        if (cannotSetUndefined) {
            const match = msg.match(/Cannot set properties of undefined \(setting '(.+)'\)/);
            const prop = match ? match[1] : null;

            // Decide array vs object case
            let kind = "object";
            if (/^\d+$/.test(prop)) {
                kind = "arrayIndex";
            }

            let userMsg = "\nYou tried to set a property on an undefined value.";
            if (kind === "arrayIndex") {
                userMsg = "\nYou tried to assign to an index of something that is undefined.\n" +
                    "This usually happens when:\n- a loop goes out of range, or\n" +
                    "- a row/item in a group wasn't created yet.";
            } else {
                userMsg = `\nYou tried to set '${prop}' on something that is undefined.\n` +
                    "Check where the variable should be initialised.";
            }

            return {
                isTad: true,
                error,
                type: "undefinedParentSet",
                message: userMsg,
            };
        }

        const pushFail = msg.endsWith("push is not a function");
        if (pushFail) {
            let userMsg = "\nYou tried to use '.push()' on something that is not an array.\n" +
                "Usually this means the variable you think is an array is actually something else.\n" +
                "Try logging it first with console.log(varName) to check its type!";
            return {
                isTad: true,
                error,
                type: "arrayPushNotFunction",
                message: userMsg,
            };
        }
     ...
     * @returns 
     */
    static determineUndefinedParent(readProp) {
        if (!readProp) {
            return "unknown";
        }

        // [0], [1], [2],  clearly indexing, almost certainly an array
        if (/^\d+$/.test(readProp)) {
            return "arrayIndex";
        }
        ...
        // You *can* get more opinionated here if you want:
        // if (["forEach","map","filter","push","pop"].includes(readProp)) return "array-method";
    ...
    static previewGroup(group, depth = 0) {
        ...
        for (let i = 0; i < loopCount; i++) {
            ...
            if (group[i] instanceof Group) {
                msg += `${tab(depth + 1)}Group(${group[i].length}):${group[i].type
                    },\n`;
