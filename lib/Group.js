...
import { QuadTree2 } from "./QuadTree2.js";
...
export class Group extends Array {
    ...
    collideSingleEntity(collideableEntity) {
        for (const sprite of this) {
    ...
    collideGroupToGroup(collideableEntity) {
        let result = false;
        for (let i = 0; i < this.length; i++) {
            const sprite = this[i];
            for (let c = 0; c < collideableEntity.length; c++) {
                const sprite2 = collideableEntity[c].id
                if (sprite.id !== sprite2.id) {
                    if (sprite.collides(collideableEntity[c])) {
                        result = true;
                    }
                }
            }
    ...
    collideSelfToSelf(collideableEntity) {
        this.#quadTree.makeTree(this);
        this.collideQuad(this.#quadTree.getTree());
        /*
        let result = false;
        for (let i = 0; i < this.length; i++) {
            const sprite = this[i];
    ...
    collides(collideableEntity) {
        if (collideableEntity instanceof Collider) {
            return this.collideSingleEntity(collideableEntity);
        }
        if (collideableEntity === this) {
            return this.collideSelfToSelf(collideableEntity);
        }
        if (collideableEntity instanceof Group) {
            return this.collideGroupToGroup(collideableEntity);
        }
        throw new Error(
            `Unsupported! ${collideableEntity}:${typeof collideableEntity}`
        );
