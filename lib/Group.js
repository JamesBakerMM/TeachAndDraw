...
export class Group extends Array {
    ...
    isValid(...elements) {
        for (const element of elements) {
            ...
            if (this.length > 0) {
                if(this[0] instanceof Collider && element instanceof Collider === false){// is a collider && element is NOT a collider
                    throw new Error(
    ...
    collides(collideableEntity) {
        ...
        if(collideableEntity instanceof Group) {
            ...
            for(let i=0; i<this.length; i++){
                const sprite=this[i];
                // console.log("ssd",sprite)
                for (const otherSprite of collideableEntity) {
                    // console.log("sprite",sprite.id,otherSprite.id,sprite.collides(otherSprite))
                    const isSameSprite=sprite.id===otherSprite.id
                    if(isSameSprite){
                        // console.log("hskjdfhkjsd")
                    }else if (sprite.collides(otherSprite)) {
                        result=true;
                    }
                }
            }
