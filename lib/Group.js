...
export class Group extends Array {
    ...
    overlapGroupToGroup(collideableEntity){
        ...
        for(let i=0; i<this.length; i++){
            ...
            for (let c = 0; c < collideableEntity.length; c++) {
                if (sprite.id!==collideableEntity[c].id && sprite.overlaps(collideableEntity[c])) {
                    result=true;
                }
            }
        }
    ...
    collideGroupToGroup(collideableEntity){
        ...
        for(let i=0; i<this.length; i++){
            const sprite=this[i];
            for (let c = 0; c < collideableEntity.length; c++) {
                if (sprite.id!==collideableEntity[c].id && sprite.collides(collideableEntity[c])) {
                    result=true;
                }
            }
        }
    ...
    collides(collideableEntity) {
        if (collideableEntity instanceof Collider) {
            return this.collideSingleEntity(collideableEntity) //we.are.LEAVING
        }
