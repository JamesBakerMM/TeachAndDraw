import { tad, make, shape, mouse, keys, text, math } from "../../lib/TeachAndDraw.js";
...
//edges
let edges = make.group();

// edges.push(makeEdge(0, tad.h / 2));
// edges.push(makeEdge(tad.w, tad.h / 2));
//end edges

//buttons
const BTN = {
    home: make.button(tad.w / 2, tad.h / 2, 100, 60, "home"),
    toPlay: make.button(tad.w / 2, tad.h / 2, 100, 60, "play"),
    toCredits: make.button(tad.w / 2, tad.h / 2, 100, 60, "credits"),
};
...
let assets = {
    ...
    bg_top: $.loadImage(0, tad.h / 2, "./images/bg.jpg"),
    bg_bottom: $.loadImage(0, tad.h, "./images/bg.jpg"),
...
function draw() {
    ...
    //top row
    bgSpace(tad.w / 2, tad.h / 2 - assets.bg.h);
    bgSpace(tad.w / 2 - assets.bg.w, tad.h / 2 - assets.bg.h);
    bgSpace(tad.w / 2 + assets.bg.w, tad.h / 2 - assets.bg.h);

    //middle row
    bgSpace(tad.w / 2, tad.h / 2);
    bgSpace(tad.w / 2 - assets.bg.w, tad.h / 2);
    bgSpace(tad.w / 2 + assets.bg.w, tad.h / 2);
...
function splashScreen() {
    text.fill = "white";
    text.print(tad.w / 2, tad.h / 2, "Press a to start!");
...
function menuScreen() {
    colour.fill = "black";
    text.print(tad.w / 2, tad.h / 2, "MENU");
    ...
    BTN.toCredits.y = tad.h / 2 + BTN.toCredits.h + 20;
}
...
function gameScreen() {
    ...
    if (enemies.length <= COUNTERS.MIN_ENEMIES) {
        ...
        makeEnemyWave(
            ...
            chosenOption.y,
            chosenOption.num,
            chosenOption.x_increment,
            chosenOption.y_increment
        );
    }
...
function edgeEnforcement() {
    ...
    const isAtRightEdge = player.x > tad.w - 10;
    if (isAtRightEdge) {
        player.x = tad.w - 10;
...
function hp() {
    ...
    colour.stroke = "rgba(0,0,0,0)";
    colour.fill = "red";
    shape.rectangle(tad.w / 2, barY, 200, 20);
    colour.fill = "green";
    shape.alignment.x = "left";
    shape.rectangle(tad.w / 2 - 100, barY, hpRectSize, 20);
}
...
function chargedShot() {
    ...
    const y = tad.w - 100;
    colour.fill = "orange";
    const chargeAngle = math.rescale(player.charge, 0, 100, 0, 360);
    const chargeSize = math.rescale(player.charge, 0, 100, 5, 25);
...
function creditScreen() {
    ...
    const credits = ["Ship Art - James", "Bg Art - Nasa"];
    colour.fill = "white";
    text.size = 40;
    text.print(tad.w / 2, tad.h / 2 - 300, "CREDITS");
    text.size = 26;

    text.alignment.x = "left";
    for (let i = 0; i < credits.length; i++) {
        const increment = i * (text.size + 10) + 40;
        text.print(tad.w / 2 - 100, tad.h / 2 - 270 + increment, credits[i]);
    }
...
function makeShot(x, y, charge) {
    ...
    let tempShot = make.boxCollider(x, y, charge, charge);
...
function makePlayer() {
    let tempPlayer = make.boxCollider(tad.w / 2, tad.h - 100, 64, 95);
...
function makeEdge(x, y) {
    let tempEdge = make.boxCollider(x, y, 30, tad.h);
    return tempEdge;
}
